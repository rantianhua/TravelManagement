...
public class PassportDaoImpl implements PassportDao {
	...
	public void add(Passport p) throws SQLException {
		...
		String sql = "insert into passport(identity,id_card,img,account_book) values(?,?,?,?)";
	...
	public void update(Passport p) throws SQLException {
		QueryRunner runner = new QueryRunner();
		String sql = "update passport set id_card=?,img=?,account_book=? where identity=?";
		Object[] param = { p.getIdCard(), p.getImg(), p.getAccountBook(),
				p.getIdentity() };
		runner.update(ManageDbUtils.getConnection(), sql, param);
	}
	...
	public Passport find(String id) throws SQLException {
		...
		return (Passport) runner.query(ManageDbUtils.getConnection(), sql, id, new BeanHandler(
				Passport.class));
	}
